...
def rearrange_string(s1, s2):
    ...
    """
    clean_s1 = parse_string(s1)
    clean_s2 = parse_string(s2)
    sum_s1 = sum((ord(c) for c in clean_s1))
    sum_s2 = sum((ord(c) for c in clean_s2))
    combined_string = clean_s1 + clean_s2
    if sum_s1 <= sum_s2:
        return ''.join(sorted(combined_string))
    else:
        return ''.join(sorted(combined_string, reverse=True))