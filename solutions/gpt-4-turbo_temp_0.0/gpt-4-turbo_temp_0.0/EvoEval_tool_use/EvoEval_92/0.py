...
def convert_strings(input_list):
    ...
    for string in input_list:
        cleaned_string = ''.join((c for c in string if c not in special_chars))
        converted_string = ''.join((num_to_word[c] if c in num_to_word else c for c in cleaned_string)).upper()
